...
bool DataOwner::verification(DataPoint q, phe::Paillier pai, vector<vector<int> > colReversePermutations, vector<vector<int> > rowReversePermutations)
{
	mpz_t maxValue;
	mpz_init(maxValue);
	mpz_set_str(maxValue, "1000000000000000", 10);  //注意：
	...
	for (int i = 0; i < FinalResult.size(); i++)
	{
		...
		if (mpz_cmp(eRect.eMinX, maxValue) > 0)  mpz_sub(eRect.eMinX, eRect.eMinX, pai.pubkey.n);
		if (mpz_cmp(eRect.eMaxX, maxValue) > 0)  mpz_sub(eRect.eMaxX, eRect.eMaxX, pai.pubkey.n);
		if (mpz_cmp(eRect.eMinY, maxValue) > 0)  mpz_sub(eRect.eMinY, eRect.eMinY, pai.pubkey.n);
		if (mpz_cmp(eRect.eMaxY, maxValue) > 0)  mpz_sub(eRect.eMaxY, eRect.eMaxY, pai.pubkey.n);
		
		x1 = mpz_get_si(eRect.eMinX);  //mpz_t 转换成 signed long 
		x2 = mpz_get_si(eRect.eMaxX);
		y1 = mpz_get_si(eRect.eMinY);
		y2 = mpz_get_si(eRect.eMaxY);
...
void DataOwner::insertPoint(DataPoint p)
{
	for (int i = 0; i < FinalResult.size(); i++)
	{
		...
		{
			Grid g = temp.result[0];
//			cout << g.rect.minX << "	" << g.rect.maxX << "	" << g.rect.minY << "	" << g.rect.maxY << endl;
//			cout << p.x << "	" << p.y << endl;
	...
	for (int i = 0; i < FinalResult.size(); i++)
	{
		...
		if (!tmp.result.empty())
		{
			...
			for (int l = 0; l < w; l++)
			{
				...
				{
					gcvt(point.x, 8, temp);  //gcvt：float 转 string 
					strcat(buf, temp);
...
//读取文件
//vector<DataPoint> readFile(char filename[])  //读NE/UN 
//{
...
vector<DataPoint> readFile(char filename[])   //读GA文件 
{
	FILE *fp;   	//文件指针
	int len;    	//行字符个数
	char buf[1024];  //缓冲区
	...
	if (fclose(fp) != 0) 
		printf("Dataset cannot be closed!\n");
	return dataPoints; 
}
...
Rectangle getDataRange(vector<DataPoint> dataPoints)
{
    ...
    {
    	// 更新矩形的边界值
        if (dataPoints[i].x < rect.minX) rect.minX = (int)dataPoints[i].x * 1.0 - 1.0;
        if (dataPoints[i].x > rect.maxX) rect.maxX = (int)dataPoints[i].x * 1.0 + 1.0;
        if (dataPoints[i].y < rect.minY) rect.minY = (int)dataPoints[i].y * 1.0 - 1.0;
        if (dataPoints[i].y > rect.maxY) rect.maxY = (int)dataPoints[i].y * 1.0 + 1.0;
	}
	return rect;
}
...
//		}	
//	}
//	serv[0].rect.maxX = -1.7;
//	serv[1].rect.maxX = -1.7;
//	serv[2].rect.minX = -1.7; serv[2].rect.maxX = 1.7;
//	serv[3].rect.minX = -1.7; serv[3].rect.maxX = 1.7;
//	serv[4].rect.minX = 1.7;
//	serv[5].rect.minX = 1.7;
//	for (int i = 0; i < SERVER_NUM; i++)
//	{
//		for (int l = 0; l < dataPoints.size(); l++) 
//		{
//			DataPoint point = dataPoints[l];
//			if (point.x >= serv[i].rect.minX && point.x < serv[i].rect.maxX 
//			 && point.y >= serv[i].rect.minY && point.y < serv[i].rect.maxY)
//			{
//			 	serv[i].dataPoints.push_back(point);
//			}
//		}
//	}
//	return;
...
void dataEncryption(Server &serv, phe::Paillier pai, string fixAesKey)
{
	...
	for (int i = 0; i < serv.Px; i++) 
	{
		for (int j = 0; j < serv.Py; j++)  
		{
		    ...
		    char buf[1024000] = {'\0'};  //开辟空间取决于要加密的字符串长度，102400目前适用于GA数据集 
		    char temp[15] = {'\0'};  //GA包含负浮点数，长度大于正浮点数，temp需要开的比NE/UN大一些 
		    int len;
			...
			for (int l = 0; l < w; l++)
			{
				...
				{
					gcvt(point.x, 7, temp);  //gcvt：float 转 string 
					strcat(buf, temp);
					strcat(buf, ",");  //分隔符"|"分隔一个点的x、y坐标 
					gcvt(point.y, 7, temp);
					strcat(buf, temp);
...
void qthread_SS(StorageServer &SS, ComputingServer CS, PaiEncPoint eq, phe::Paillier pai)
{
	...
	mpz_t maxValue;
	mpz_init(maxValue);
	mpz_set_str(maxValue, "1000000000000000", 10);
