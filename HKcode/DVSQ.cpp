...
vector<DataPoint> readFile(char filename[])   //读GA/HK文件 
{
	FILE *fp;   	//文件指针
	int len;    	//行字符个数
	char buf[1024];  //缓冲区
	...
	while (fgets(buf,1024, fp) != NULL)
	{
		DataPoint point; 
	 	...
	 	len = strlen(buf);
	 	buf[len-1] = '\0';   //去掉换行符 
	 	for (i = 0; i < len; i++)  //用于跳过HK的ID列，读入GA时注释掉这段for循环 
		{
			if (buf[i] == '	')
				break;	
		}
		i += 2;  //定位到HK的x坐标列，读入GA时注释掉这一行 
		char tempX[10] = {'\0'};   	
		for (j = 0; j < 10; j++)
		{
			if (buf[i] == '	')  //tab键 
...
void dataEncryption(Server &serv, phe::Paillier pai, string fixAesKey)
{
	...
	for (int i = 0; i < serv.Px; i++) 
	{
		for (int j = 0; j < serv.Py; j++)  
		{
		    ...
		    char buf[3000000] = {'\0'};  //开辟空间取决于要加密的字符串长度，102400目前适用于HK数据集 
...
{
//	cout << "111:" << serv.gridSet[1][20].eP << endl;
	for ( int i = 0; i < serv.Px; i++)
	{
		for (int j = 0; j < serv.Py; j++)
		{
...
//			string gridStr(serv.gridSet[i][j].eP);
//			if (i == 1 && j == 20)
//			{
//				cout << "gridStr = " << gridStr << endl;
//				serv.gridSet[i][j].eP[0] = '1';
//				cout << "gridStr = " << gridStr << endl;
//			}
...
//			if (i == 1 && j == 21)
//				cout << "222:" << serv.gridSet[1][20].eP << endl;
		}
	}
//	cout << "333:" << serv.gridSet[1][20].eP << endl;
	return; 
}
