...
//				cout << "maxY = " << maxY << endl;
//				cout << "minY = " << minY << endl;
//				cout << "result.size = " << temp.result.size() << endl;
//				for (int l = 0; l < temp.result.size(); l++)
//				{
//					Grid g = temp.result[l];
//					cout << l << ": p.x = " << g.p.x << endl;
//					cout << l << ": p.y = " << g.p.y << endl;
//				}
...
void dataEncryption(Server &serv, phe::Paillier pai, string fixAesKey)
{
	...
	for (int i = 0; i < serv.Px; i++) 
	{
		for (int j = 0; j < serv.Py; j++)  
		{
		    ...
		    char buf[2048000] = {'\0'};  //开辟空间取决于要加密的字符串长度，102400目前适用于UN数据集 
		    char temp[15] = {'\0'};
		    int len;
