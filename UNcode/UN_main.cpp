...
int main()
{
	...
	char filename[20] = "UN/UN3.txt";
	vector<DataPoint> dataPoints = readFile(filename);  //读入数据集 
	cout << dataPoints.size() << endl;
	Rectangle rect = getDataRange(dataPoints);  //获取数据集所在大矩形 
...
//	StorageServer SS8(serv[8].eRect, serv[8].gridSet);
	...
	for (int i = 0; i < SERVER_NUM; i++)  //不用的指针都置空，防止出现野指针 
	{
		for (int j = 0; j < serv[i].Px; j++)
		{
			...
			{
				serv[i].gridSet[j][k].eDataPoints = nullptr;
				serv[i].gridSet[j][k].eP = nullptr;
			}
		}
		vTable[i].eHash = nullptr;
	}
	...
	//===============================================================================//
	//--------------------------------范围查询过程-----------------------------------//
	char fname[5][50] = {"UN/UN_rect_testset1.txt", "UN/UN_rect_testset2.txt", "UN/UN_rect_testset3.txt", "UN/UN_rect_testset4.txt", "UN/UN_rect_testset5.txt"};
	char res_name[5][20] = {"UN/res_test1.txt", "UN/res_test2.txt", "UN/res_test3.txt", "UN/res_test4.txt", "UN/res_test5.txt"};
//	char fname[1][50] = {"UN/UN_rect_testset_pre.txt"};
//	char res_name[1][30] = {"UN/res_lambda_5_m_4.txt"};
...
//					cout << "nonResNum = " << res.second << endl;
					...
					cout << j << ": The Range Query is failed!" << endl;
				//清空结果集 
				SS0.setEmpty();
				SS1.setEmpty();
				SS2.setEmpty();
...
//				SS8.setEmpty();
	...
	//===============================================================================//*/
	//-------------------------------点查询-插入过程---------------------------------//
	/*char fname[50] = "UN/UN_point_testset.txt";
	char rname1[50] = "UN/res_insert.txt";
	FILE *fp;
...
//	//===============================================================================//
//	//-------------------------------点查询-删除过程---------------------------------//
//	char rname2[50] = "UN/delete_res_test.txt";
